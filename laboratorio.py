...
NAME_X, AGE_X, DATE_X, TEXT_X = 1.3 * cm, 10.5 * cm, 14.7 * cm, 3.3 * cm
...
class LaboratorioApp:
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(config, text="Tamaño letra PDF:").pack(side="left", padx=10)
        ...
        # Selector solo para la columna de la IZQUIERDA (Estudios)
        self.lab_selector = ctk.CTkOptionMenu(self.tab_form, values=["-- Historial de Estudios --"], command=self._on_lab_selected)
        self.lab_selector.pack(fill="x", padx=20, pady=5)

        # Textos
        txt_labels = ctk.CTkFrame(self.tab_form, fg_color="transparent")
        txt_labels.pack(fill="x", padx=20)
        ctk.CTkLabel(txt_labels, text="Estudios (Seleccionable por fecha)", font=("Helvetica", 12, "bold")).pack(side="left", padx=5)
        ctk.CTkLabel(txt_labels, text="Historial Completo de Notas Internas", font=("Helvetica", 12, "bold"), text_color="#3498db").pack(side="right", padx=120)

        txt_f = ctk.CTkFrame(self.tab_form, fg_color="transparent")
        txt_f.pack(fill="both", expand=True, padx=20, pady=5)
        
        # Columna 1: Estudios
        self.txt_estudios = ctk.CTkTextbox(txt_f)
        self.txt_estudios.pack(side="left", fill="both", expand=True, padx=5)
        self.txt_estudios.insert("1.0", TEXTO_PRECARGADO)
        
        # Columna 2: Todas las Notas
        self.txt_notes = ctk.CTkTextbox(txt_f, fg_color="#1a1a1a", text_color="#ecf0f1")
        self.txt_notes.pack(side="right", fill="both", expand=True, padx=5)
    ...
    def load_selected(self):
        ...
        self.ent_name.insert(0, p[1])
        self.ent_age.insert(0, p[2])
        
        # Cargar historial de estudios para el selector
        self._refresh_lab_selector(p_id)
        # Cargar TODAS las notas de recetas en la columna derecha
        self._load_all_internal_notes(p_id)
        
        self.tabs.set("Nueva Solicitud")

    def _load_all_internal_notes(self, pid):
        """Carga todas las notas de todas las recetas del paciente en la columna derecha."""
        self.txt_notes.delete("1.0", "end")
        with sqlite3.connect(DB_NAME) as conn:
            notes_data = conn.execute("SELECT fecha, notes FROM recetas WHERE patient_id=? ORDER BY id DESC", (pid,)).fetchall()
        
        if notes_data:
            content = ""
            for fecha, nota in notes_data:
                if nota and nota.strip():
                    content += f"--- FECHA: {fecha} ---\n{nota.strip()}\n\n"
            self.txt_notes.insert("1.0", content if content else "El paciente no tiene notas registradas.")
        else:
            self.txt_notes.insert("1.0", "No se encontraron recetas para este paciente.")
    ...
    def save_and_print(self):
        ...
        with sqlite3.connect(DB_NAME) as conn:
            ...
            conn.execute("INSERT INTO laboratorio (patient_id, fecha, estudios) VALUES (?,?,?)", 
                         (self.current_patient_id, self.ent_date.get(), self.txt_estudios.get("1.0", "end-1c")))
    ...
    def generate_pdf(self, name, age, date, estudios):
        ...
        t.setFont(font_name, int(self.opt_size.get()))
    ...
    def _refresh_lab_selector(self, pid):
        ...
        self.lab_selector.configure(values=[r[0] for r in regs] if regs else ["-- Sin estudios previos --"])

    def _on_lab_selected(self, val):
        """Actualiza solo la columna de estudios al seleccionar una fecha."""
        for r in self._lab_cache:
            if r[0] == val:
                self.txt_estudios.delete("1.0", "end")
                self.txt_estudios.insert("1.0", r[1])
                break
