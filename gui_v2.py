...
class MedicalApp:
    ...
    def _build_tab_images(self):
        ...
        ctk.CTkLabel(obs_frame,
                     text="Análisis de Imagen / Observaciones",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        self.txt_img_desc = ctk.CTkTextbox(obs_frame, height=80)
    ...
    def _draw_header(self, c, h):
        """
        Switch OFF → solo valores sin etiquetas
        Switch ON  → Nombre: / Edad: / Fecha:
        Cuerpo siempre limpio, sin etiquetas.
        """
        ...
        if show:
            ...
            if age:
                c.drawString(AGE_X,  y, f"Edad: {age}")
            if date:
                c.drawString(DATE_X, y, f"Fecha: {date}")
        else:
            c.drawString(NAME_X, y, name)
            if age:
                c.drawString(AGE_X,  y, age)
            if date:
    ...
    def generate_patient_pdf(self):
        ...
        # Cuerpo: solo el texto, sin ninguna etiqueta
        c.setFont("Helvetica", fs)
    ...
    def generate_image_pdf(self):
        ...
        # Texto de observaciones — solo el texto, sin etiquetas
        c.setFont("Helvetica", fs)
