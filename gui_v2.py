...
DATE_X          = 2.5 * cm + 10   * cm + 3 * cm
HEADER_BODY_GAP = 1   * cm

# Grilla 2×2 colposcopia
IMG_W      = 7.5 * cm
IMG_H      = 6.0 * cm
COLS       = 2
ROWS       = 2
NUM_IMAGES = COLS * ROWS   # 4
...
class MedicalApp:
    ...
    def _build_tab_form(self):
        ...
        ctk.CTkLabel(name_frame, text="Nombre del Paciente",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        self.ent_name = ctk.CTkEntry(name_frame,
                                     placeholder_text="Ej: Juan Pérez",
                                     width=400)
        self.ent_name.pack(anchor="w", pady=(2, 0))

        # Año de nacimiento + Edad + Fecha
        ...
        ctk.CTkLabel(dob_frame, text="Año de Nacimiento",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        self.ent_birth_year = ctk.CTkEntry(dob_frame,
                                           placeholder_text="Ej: 1990",
                                           width=120)
        ...
        ctk.CTkLabel(age_frame, text="Edad",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        self.ent_age = ctk.CTkEntry(age_frame,
        ...
        ctk.CTkLabel(date_frame, text="Fecha",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(meds_frame, text="Medicamentos e Indicaciones",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(notes_frame, text="Notas Privadas (Solo Uso Interno)",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(pdf_options_frame, text="Opciones de PDF",
    ...
    def _build_tab_images(self):
        ...
        for i in range(NUM_IMAGES):
            r, c = divmod(i, COLS)
            slot = ctk.CTkFrame(self.img_grid, border_width=2,
                                border_color="#555")
            ...
            ctk.CTkButton(b_frame, text="Agregar", width=90,
                          command=lambda idx=i: self.add_image(idx)).pack(
                              side="left", padx=2)
            ctk.CTkButton(b_frame, text="Eliminar", width=90,
                          fg_color="#c0392b",
        ...
        ctk.CTkLabel(obs_frame, text="Observaciones del Estudio",
                     font=("Helvetica", 12, "bold")).pack(anchor="w")
    ...
    def load_selected(self):
        ...
        with sqlite3.connect(DB_NAME) as conn:
            row = conn.execute(
                "SELECT * FROM patients WHERE id=?", (p_id,)).fetchone()
        if row:
            self.clear_form()
    ...
    # ── HELPER: word-wrap ─────────────────────────────────────────────────────
    def _draw_wrapped_text(self, c, text, x, y, max_width, font_name,
                           font_size, line_height, page_h):
        """
        Dibuja texto con word-wrap dentro de max_width.
        Devuelve la Y final tras escribir todo el texto.
        """
    ...
    def generate_image_pdf(self):
        ...
        Layout fiel a la referencia:
          - nombre (izq) | fecha (der)
          - "Estudio de Colposcopia" centrado negrita
          - Grilla 2×2 centrada  (7.5 cm × 6 cm por celda, sin gap)
          - Texto de observaciones con word-wrap debajo de la grilla
        """
        name     = self.ent_name.get()
        date     = self.ent_date.get()
        ...
        # ── 1. Cabecera: nombre izq / fecha der ───────────────────────────────
        y_header = h - TOP_MARGIN
        c.setFont("Helvetica", fs)
        c.drawString(MARGIN_LEFT, y_header, name)             # nombre izquierda
        c.drawRightString(w - MARGIN_RIGHT, y_header, date)   # fecha derecha

        # ── 2. Título centrado en negrita ─────────────────────────────────────
        y_title = y_header - (1.0 * cm)
        c.setFont("Helvetica-Bold", fs + 1)
        c.drawCentredString(w / 2, y_title, "Estudio de Colposcopia")

        # ── 3. Grilla 2×2 centrada ────────────────────────────────────────────
        total_grid_w = COLS * IMG_W          # 2 × 7.5 = 15 cm
        grid_x       = (w - total_grid_w) / 2
        grid_top     = y_title - (0.5 * cm)

        valid = []
        for fname in self.image_paths:
            fpath = os.path.join(MEDIA_FOLDER, fname) if fname else None
            valid.append(fpath if fpath and os.path.exists(fpath) else None)

        for idx in range(NUM_IMAGES):
            row = idx // COLS
            col = idx %  COLS
            ix  = grid_x + col * IMG_W
            iy  = grid_top - (row + 1) * IMG_H   # ReportLab: Y hacia arriba

            if valid[idx]:
                c.drawImage(valid[idx], ix, iy,
            ...
            # Borde negro sobre cada celda (siempre)
        ...
        # ── 4. Observaciones con word-wrap ────────────────────────────────────
        if desc_txt.strip():
            y_obs     = grid_top - ROWS * IMG_H - (0.8 * cm)
